...
def edit_user(user_id: int, user_update: schemas.UserUpdate, db: Session = Depends(get_db), current_regular_user: dict = Depends(get_current_regular_user_with_options)):
...
def remove_user(user_id: int, db: Session = Depends(get_db), current_regular_user: dict = Depends(get_current_regular_user_with_options)):
...
def add_competencia(user_id: int, competencia_id: int, db: Session = Depends(get_db), current_regular_user: dict = Depends(get_current_regular_user_with_options)):
    added = functions.add_competencia_to_user(db, user_id, competencia_id)
    return added

@router.delete("/{user_id}/competencias/{competencia_id}")
def remove_competencia(user_id: int, competencia_id: int, db: Session = Depends(get_db), current_regular_user: dict = Depends(get_current_regular_user_with_options)):
...
def get_me(db: Session = Depends(get_db), current_regular_user: dict = Depends(get_current_regular_user_with_options)):
