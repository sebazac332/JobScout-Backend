...
def create_competencia(competencia: schemas.CompetenciaCreate, db: Session = Depends(get_db), all_users: dict = Depends(get_current_user)):
    return functions.create_competencia(db, competencia)

@router.delete("/{competencia_id}", response_model=schemas.Competencia)
def delete_competencia(competencia_id: int, db: Session = Depends(get_db), all_users: dict = Depends(get_current_user)):
